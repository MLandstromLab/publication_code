...
prepareHeatmap = function(feature_data, 
  ...
  if ((cn.features.present == T) | (exp.features.present == T)) {
        ...
        gleason.score = feature_data$Gleason.group.clin
        ...
        if (is.null(feature_data$Gleason.group.clin) == F) {
            gleasonCol = structure(c("seagreen3","tomato2"), names = c("Gleason_group_1","Gleason_group_2"))
        }

        # Prepare the heatmap components  

        # Collect varibles into a final data frame 
        heatmap.data = data.frame(Time = survivalData$time,
