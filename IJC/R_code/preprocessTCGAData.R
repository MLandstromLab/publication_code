...
determineGleasonGroup = function(primary.patt, secondary.patt){
    ...
    if (gleason.score < 7){
       return("Gleason group 1") 
    } else if (gleason.score > 7) {
       return("Gleason group 2") 
    } else {
        if (primary.score == 4) {
            return("Gleason group 2")
        } else {
...
prepSummaryExp = function(x, raw.data, variables, type){
    ...
    raw.data = raw.data %>%
                        dplyr::select(one_of(rownames(x)))
...
generateModelMatrices = function(train.data, 
    ...
    x.valid.mat = x.valid.mat[, colnames(x.valid.mat) != "Genderfemale"]
    ...
    x.valid.mat = x.valid.mat[, colnames(x.valid.mat) != "Gleason.groupGleason group 1"]
